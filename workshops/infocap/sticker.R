...
p <- ggplot(data.frame(x = c(-4, 6)), aes(x)) + 
  mapply(function(mean, sd, col) {
    stat_function(fun = dnorm, args = list(mean = mean, sd = sd), col = col, size = 2)
  }, 
...
sticker(p, package="InfoCap", p_family = "AlfaSlabOne-Regular", p_color = "black", p_size=9, s_x=1, s_y=.75, s_width=1,
